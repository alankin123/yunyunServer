...
import com.alankin.common.api.ILogApi;
import com.alankin.common.global.MapCache;
import com.alankin.common.service.RouteService;
import com.alankin.common.threadPool.CenterThreadPool;
...
public class BusinessLogHandler extends ApplicationObjectSupport {
	...
	public void initConfig() {
		...
		try {
			if (getApplicationContext().getBean("centerBusinessLog") == null) {
				logger.info("没有配置日志服务消费者，跳过加载。");
				return;
			}
		} catch (Exception e) {
			logger.info("没有配置日志服务消费者，跳过加载。");
	...
	private Map<String, String> getEntityFieldNameMap(String serviceName) {
		...
		try {
			...
			for (Field field : entity.getDeclaredFields()) {
				...
				com.alankin.common.vo.field param = field.getAnnotation(com.alankin.common.vo.field.class);
				if (param == null) {
